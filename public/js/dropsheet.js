...
var DropSheet = function DropSheet(opts) {
	...
	function process_wb(wb, type, sheetidx) {
		if (wb.SheetNames.length >= 2) {
			...
			var json2 = to_json(wb, type)[sheet], cols2 = get_columns(wb.Sheets[sheet], type);
			...
			opts.on.sheet(json, json2, cols, cols2, wb.SheetNames, choose_sheet);			
		} else {
	...
	function handleDrop(e) {
		...
		// files = $('#lefile').prop('files');

		$('#drops').val(files[0].name);

		var ext = $('#drops').val().split('.').pop().toLowerCase();
		if($.inArray(ext, ['xls','xlsx']) != -1) {

			// Renew the error list
			errorCells.error_data = [];
			errorStepCells.error_data = [];
			...
			$('#drops').val('');
			alertify.alert('Error: This is not a valid Excel file. [*.xls, *.xlsx]', function(){});
		}
	}
	...
	function handleChange(e) {
		var files = $('input[id=drop]').prop('files');	
		//alert(files[0].name);
	...
	if(opts.drop.addEventListener) {
		...
		opts.drop.addEventListener('change', handleChange, true);
	}
